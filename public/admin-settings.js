(() => {
  ...
  function cacheEls() {
    [
      'settings-nav','settings-sidebar','settings-menu-toggle','settings-save-state','permanent-admin-email',
      'metric-capabilities','metric-enabled','metric-security','metric-infrastructure','identity-list',
      'settings-search','settings-category-filter','settings-status-filter','settings-result-count','settings-grid',
      'settings-load-more','diagnostics-grid','settings-audit','settings-toast','setting-theme','setting-density',
      'setting-scale','setting-scale-value','setting-reduced-motion','setting-high-contrast','setting-passkey-critical',
      'setting-new-device','setting-session-hours','setting-idle-minutes','save-core-settings','save-capability-settings',
      'bulk-enable-visible','bulk-disable-visible','refresh-diagnostics','refresh-audit','settings-export',
      'settings-export-side','settings-reset'
  ...
  function updateCapabilitySaveButton() {
    const count = Object.keys(stagedCapabilities).length;
    const button = els['save-capability-settings'];
    if (!button) return;
    button.disabled = count === 0;
    button.textContent = count ? `Save ${count} change${count === 1 ? '' : 's'}` : 'All capability changes saved';
    button.classList.toggle('settings-primary', count > 0);
  }
  ...
  function renderSidebar() {
    const buttons = [`<button type="button" data-category=""${activeCategory === '' ? ' class="active"' : ''}>All settings <span>1000</span></button>`];
    catalog.forEach((group) => {
      const active = group.category === activeCategory ? ' class="active"' : '';
      buttons.push(`<button type="button" data-category="${escapeHtml(group.category)}"${active}>${escapeHtml(group.category)} <span>20</span></button>`);
    });
  ...
  function renderSettings() {
    ...
    els['settings-grid'].querySelectorAll('[data-setting-toggle]').forEach((input) => input.addEventListener('change', () => {
      ...
      const count = Object.keys(stagedCapabilities).length;
      setSaveState(`${count} capability change${count === 1 ? '' : 's'} staged`);
      renderSettings();
      renderMetrics();
      updateCapabilitySaveButton();
    }));
    updateCapabilitySaveButton();
  }
  ...
  function renderMetrics() {
    ...
    const checks = [security.googleOnly, security.bruteForceProtection, security.suspiciousLoginBlocking, security.mfaCriticalActions];
    els['metric-security'].textContent = `${checks.filter(Boolean).length}/${checks.length}`;
    const values = Object.values(diagnostics?.bindings || {});
    els['metric-infrastructure'].textContent = values.length ? `${values.filter(Boolean).length}/${values.length}` : '—';
  }
  ...
  function renderDiagnostics() {
    const cards = [];
    const d = diagnostics || {};
    Object.entries(d.bindings || {}).forEach(([name, ok]) => cards.push({
      name: name === 'googleClient' ? 'Google OAuth Client' : name === 'cloudRun' ? 'Google Cloud Run' : name.toUpperCase(),
      ok,
      detail: ok ? 'Connected / available' : 'Not detected'
    }));
    Object.entries(d.security || {}).forEach(([name, ok]) => cards.push({
      name: name.replace(/([A-Z])/g, ' $1').replace(/^./, (c) => c.toUpperCase()),
      ok: Boolean(ok),
      detail: typeof ok === 'boolean' ? (ok ? 'Enforced' : 'Not active') : text(ok)
    }));
  ...
  async function saveCore() {
    setSaveState('Saving…');
    try {
      const result = await request(api, { method: 'PUT', body: JSON.stringify({
        appearance: {
  ...
  async function saveCapabilities() {
    ...
    if (!entries.length) {
      toast('There are no staged capability changes to save.');
      return;
    }
  ...
  function stageVisible(value) {
    ...
    const count = Object.keys(stagedCapabilities).length;
    setSaveState(`${count} capability changes staged`);
    renderSettings();
    renderMetrics();
    updateCapabilitySaveButton();
    toast(`${count} capability changes staged. Review them, then press Save.`);
  }
  ...
  async function resetSettings() {
    if (!window.confirm('Reset configurable Admin Settings to safe defaults? Permanent superadmin access and mandatory security policies will remain protected.')) return;
    const reason = window.prompt('Reason for reset (required for governance):', 'Restore safe Admin Settings defaults') || '';
    if (!reason.trim()) {
      toast('Reset cancelled because a governance reason is required.', true);
      return;
    }
    try {
  ...
  function bindEvents() {
    ...
    els['settings-category-filter'].addEventListener('change', () => {
      activeCategory = els['settings-category-filter'].value;
      visibleLimit = 120;
      renderSidebar();
      renderSettings();
    });
    ...
    els['save-capability-settings'].addEventListener('click', saveCapabilities);
    ...
    window.addEventListener('beforeunload', (event) => {
      if (Object.keys(stagedCapabilities).length) {
