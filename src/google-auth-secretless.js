...
async function createOauthFlow(env) {
  ...
  await env.CMS_KV.put(`auth:google-oauth:${state}`, JSON.stringify({ nonce, codeVerifier, createdAt: Date.now() }), {
...
async function googleOauthStart(request, env) {
  ...
  if (!clientId) return oauthErrorPage('Google Sign-In is not configured yet.', 503);
...
async function exchangeAuthorizationCode(request, env, code, flow) {
  ...
  const optionalSecret = String(env.GOOGLE_OAUTH_CLIENT_SECRET || '').trim();
  if (optionalSecret) body.set('client_secret', optionalSecret);
...
async function googleOauthCallback(request, env) {
  ...
  try {
    ...
    console.error(JSON.stringify({ type: 'google_admin_oauth_secretless_error', message: String(error?.message || error) }));
