...
// Public bootstrap verifier only. The plaintext administrator password is never stored in source.
// After the first successful sign-in for an account, a fresh per-account salt/verifier is stored in KV.
...
async function bootstrapMatches(password) {
  ...
  const verifier = await deriveVerifier(password, BOOTSTRAP_CREDENTIAL.salt, BOOTSTRAP_CREDENTIAL.iterations);
...
async function verifySession(env, token) {
  ...
  let data;
  try { data = JSON.parse(decodeBase64urlText(payload)); }
  catch (_) { return null; }
...
export async function login(request, env) {
  ...
  if (!env.CMS_KV) return json({ error: "The admin storage binding is not available on this Worker." }, 503);
  ...
  if (account && record) {
    valid = await credentialMatches(password, record);
  } else if (account && await bootstrapMatches(password)) {
    record = await makeCredential(password, 1);
  ...
  if (!token) return json({ error: "Could not create administrator session." }, 500);
...
export async function confirmReset(request, env) {
  ...
  if (passwordProblem) return json({ error: passwordProblem }, 400);
  ...
  await storeCredential(env, email, await makeCredential(newPassword, version));
