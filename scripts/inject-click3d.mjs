...
if(!source.includes('/touch-60.css')){
  const anchor='<link rel="stylesheet" href="/click-advanced40.css">';
  if(!source.includes(anchor))throw new Error('touch 60 injector: advanced 40 style anchor not found');
  source=source.replace(anchor,`${anchor}<link rel="stylesheet" href="/touch-60.css">`);
  console.log('touch 60 injector: public styles wired');
}
...
if(!source.includes('/touch-60-guard.js')){
  const anchor='<script src="/click-advanced40.js" defer></script>';
  if(!source.includes(anchor))throw new Error('touch 60 guard injector: advanced 40 runtime anchor not found');
  source=source.replace(anchor,`${anchor}<script src="/touch-60-guard.js" defer></script>`);
  console.log('touch 60 guard injector: public runtime wired');
}

if(!source.includes('/touch-60.js')){
  const anchor='<script src="/touch-60-guard.js" defer></script>';
  if(!source.includes(anchor))throw new Error('touch 60 injector: guard runtime anchor not found');
  source=source.replace(anchor,`${anchor}<script src="/touch-60.js" defer></script>`);
  console.log('touch 60 injector: public runtime wired');
}
